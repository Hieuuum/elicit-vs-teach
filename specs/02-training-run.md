...
| G5 | runs 3, 4 | Zero/16-shot operator add/sub. Expectation: A ~2%/12%, B 0%/0% — the only remaining independent regime evidence. Protocol revision (owner 2026-07-22, after the OPEN(2) pilots): eval questions and exemplar shots are drawn exclusively from the eval-reserved tail of the frozen D_target order (rows ≥ 900,000, `D_TARGET_EVAL_RESERVED_START`); `train_target.py` refuses any prefix reaching into the reserve (null = full file included), and `gates.py g5` refuses to score a run whose manifest records a D_target prefix beyond it. Question texts in the frozen file are globally unique (verified 2026-07-22 on the pinned order_hash), so index disjointness is question-level disjointness. The original full-file draw overlapped pilot training prefixes (1.2%–50.6% of eval questions at n10k–n500k; measured accuracy inflation ≤ 0.4 points) |
