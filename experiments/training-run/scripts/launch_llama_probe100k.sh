#!/usr/bin/env bash
# Llama-3.2-1B LEARNING-CURVE PROBE (100K scale) — matched pair over the FIRST
# 100,000 target examples, differing ONLY in whether the run-9 format installer
# precedes the target stage:
#   noinst (elicit) : base Llama-3.2-1B          -> D_target[:100000]
#   inst   (teach)  : run-9 installer merged     -> D_target[:100000]
# batch 8 -> 12,500 steps, ONE pass, NO convergence stop (stop_reason=max_steps
# is intentional). The learning curve rides train_target.py's log-spaced curve
# scheduler (~256 dense-early val points); see the config headers for the eval-
# cadence arithmetic. This is a learning-CURVE probe, NOT an EDL measurement —
# do not report the result as an EDL ratio. Runs ON THE BOX only (--confirm-cost).
#
...
[[ " $* " == *" --confirm-cost "* ]] || {
  echo "launch_llama_probe100k.sh: --confirm-cost required (budget rule)" >&2
  exit 1
}
ARM=all
STAGE=all
...
[[ $STAGE != push || $PUSH == 1 ]] || {
  echo "launch_llama_probe100k.sh: --stage push requires --push" >&2; exit 1; }
...
NOINST_RID=evt-llama-probe100k-noinst
INST_RID=evt-llama-probe100k-inst
...
train_arm() {
  ...
  case $rid in
    "$NOINST_RID")
      ...
      python3 train_target.py --config ../configs/llama_probe100k_noinst.yaml \
    ...
    "$INST_RID")
      ...
      python3 train_target.py --config ../configs/llama_probe100k_inst.yaml \
