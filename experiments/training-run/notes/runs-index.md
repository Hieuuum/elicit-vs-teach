...
| evt-llama-fig2nl-installer | fig2nl NL-replication installer, LoRA r512/α32 @ 3.53e-4 on row 0 of `D_dose_mult` (operator-notation MULT dose); shared parent of the inst arm. Absorption 0.000236 PASS, G4-on-NL-prompts 0.9609 PASS recorded; **G2 0.1719 measured but de-gated by the owner** (`gate_measured_not_gating`, no verdict written) — the inst arm's handicap | canonical | both — metadata only; **no weights anywhere** (decision 10 metadata-only push, box destroyed 2026-08-04) | decisions.md 2026-08-04 "fig2nl STOPPED"; EXPERIMENTS.md §6.11 |
| evt-llama-fig2nl-noinst-n1000 … -n1000000 (19 runs: n=1000, 1468, 2154, 3162, 4642, 6813, 10000, 14678, 21544, 31623, 46416, 68129, 100000, 146780, 215443, 316228, 464159, 681292, 1000000) | fig2nl NL-replication sweep, noinst arm, base Llama → fresh LoRA r512/α32 → `D_algo[:n]`, one run per size — **arm COMPLETE, all 19 converged** | canonical | both — metadata only; **no weights anywhere** (decision 10 + teardown 2026-08-04) | decisions.md 2026-08-04 "fig2nl STOPPED"; EXPERIMENTS.md §6.11 |
| evt-llama-fig2nl-inst-n1000 … -n100000 (**13 runs** of the planned 19: n=1000 … 100000) | fig2nl NL-replication sweep, inst arm, merged installer checkpoint → fresh LoRA r512/α32 → `D_algo[:n]`, G7 data-order matched to its noinst twin (arm-serial ascending, noinst fully first) — **arm TRUNCATED**, all 13 converged; gives 13 matched noinst/inst pairs | canonical | both — metadata only; **no weights anywhere** (decision 10 + teardown 2026-08-04) | decisions.md 2026-08-04 "fig2nl STOPPED"; EXPERIMENTS.md §6.11 |
| evt-llama-fig2nl-inst-n146780, -n215443, -n316228, -n464159, -n681292, -n1000000 (6 runs) | fig2nl inst arm, the 6 sizes above n=100,000 — **never launched**: the owner stopped the sweep 2026-08-04 and the box was destroyed. Not resumable in place; re-running the family is the only route | planned (→ cancelled 2026-08-04) | none (never launched) — no run dir anywhere, no manifest, so no spec 00 §2 lifecycle field exists; distinct from **lost** | decisions.md 2026-08-04 "fig2nl STOPPED"; EXPERIMENTS.md §6.11 |
| evt-ts38-pretaught-parent | ts38 mini pre-taught parent, full FT on `D_target` (paper App. E protocol, lr 3e-4 run-2 pin); gated on G1 (capability) + G8 (TinyStories retention, bar OPEN) | planned | none (planned) — not yet launched, no run dir anywhere yet | EXPERIMENTS.md §6.14; decisions.md 2026-08-14 "family built" |
| evt-ts38-base-n1000 … -n316228 (5 runs: n=1000, 4642, 21544, 100000, 316228) | ts38 mini Arm A (teach), init=base, LoRA r128/α32 @1e-3 on `D_algo_bare` frozen order `946b5d02…` | planned | none (planned) — not yet launched, no run dir anywhere yet | EXPERIMENTS.md §6.14; decisions.md 2026-08-14 "family built" |
| evt-ts38-pretaught-n1000 … -n316228 (5 runs, same sizes as Arm A) | ts38 mini Arm B (elicit), init=parent, identical LoRA recipe + data order — arms differ only in θ0 (G7-matched to Arm A) | planned | none (planned) — not yet launched, no run dir anywhere yet | EXPERIMENTS.md §6.14; decisions.md 2026-08-14 "family built" |
