...
Every target run already records the PRE-CLIP global gradient norm and
per-module norms at every update (geode.edl.loop._gradstat ->
runs/<rid>/logs/gradstats.jsonl, spec 00 §4) — no proxy, no rerun.
...
def load_run(run_id: str):
    ...
    if not steps:
        raise SystemExit(f"[grad] no gradstats for {run_id} under {STORE}")
...
def summarize(label: str, steps, g, grp) -> dict:
    ...
    out = {
        ...
        **{f"mass_share_{k}": float((v**2).sum() / total_sq) for k, v in grp.items()},
    }
    print(f"[grad] {label:<10} steps {n:>6}  peak {out['peak_norm']:.3f} @ {out['peak_step']}"
          ...
          f"  Σ‖g‖ {mass:.1f} ({mass / n:.4f}/step)"
          f"  share QK/VO/MLP {out['mass_share_QK']:.2f}/{out['mass_share_VO']:.2f}/{out['mass_share_MLP']:.2f}")
    return out
