"""Unit test for ``train_target.lora_adapter_state_dict`` (owner directive
2026-07-31: every LoRA target run must also write a compact adapter-only
...
checkpoint or tokenizer needed.
"""
...
def test_lora_adapter_state_dict_keeps_only_a_and_b_weights() -> None:
    ...
    adapter = train_target.lora_adapter_state_dict(state_dict)
