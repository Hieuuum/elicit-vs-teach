...
FULL_FT = [
    ...
    "ts1b_fig2ts_installer.yaml",
    "ts1b_fig2ts_installer_bare.yaml",
]
EMBEDDING_WARMSTART = [
...
LORA_TARGET = [
    ...
    ("ts1b_fig2ts_noinst.yaml", None),
    ("ts1b_fig2ts_inst.yaml", None),
...
# =============================================================================
# ts38dense -- target-size grid density extension (EXPERIMENTS.md §6.21;
# decisions.md 2026-08-21 "ts38dense pre-registration"): densifies the
# base/pp/fs(i=1000,s=316) target-size grid from 5 to 10 log-spaced points by
# adding 5 NEW sizes {2154, 10000, 46416, 146780, 215443} on top of the 5
# already-shipped {1000, 4642, 21544, 100000, 316228}. Three arms per new
# size, trained base-first (pp/fs pair against the base run as their G7
# anchor / theta0 parent respectively):
#   base (evt-ts38-base-n<N>)        -- ts38_base_n<N>.yaml, ALREADY
#       COMMITTED for the shelved ts38grid family (EXPERIMENTS.md §6.17) and
#       reused here VERBATIM -- not edited, not retrained, not re-listed in
#       any list above (this section only reads those files, it does not
#       redeclare them).
#   pp   (evt-ts38pp-pretaught-n<N>) -- NEW ts38pp_pretaught_n<N>.yaml
#       overlays, same shape as ts38pp_pretaught_n4642.yaml above; theta0 =
#       evt-ts38pp-parent.
#   fs   (evt-ts38fs-i1000-n<N>-s316) -- NEW ts38fs_dense_i1000_n<N>.yaml
#       overlays over ts38fs_target.yaml; theta0 = evt-ts38fs-parent-n1000
#       (the i=1000 LoRA install parent, merged). HAND-COMMITTED, not
#       generated by generate_ts38fs_overlays.py (this is 5 cells at one
#       install/seed, not the full 55-cell grid), so the filename uses the
#       `ts38fs_dense_` prefix (precedent: `ts38fs_tiny_i*_n*.yaml`) rather
#       than the generator's own `ts38fs_i<i>_n<n>_s<s>.yaml` pattern --
#       launch_ts38fs_family.sh:501 hard-fails unless EXACTLY 55 files match
#       that glob, so a 56th match here would break that guard. The RUN ID
#       still uses the canonical ts38fs-proper pattern (this cell IS a
#       ts38fs cell: same parent, same recipe, same seed, only n is new).
#       No G7 pairing for this arm, same as the rest of ts38fs (see
#       ts38fs_target.yaml's own "NO G7 PAIRING" header note).
# Pins per size are IDENTICAL across all three arms -- only theta0 differs,
# same "arms differ only in theta0" invariant as every other ts38(*) family.
# Purely additive: this section does not touch any list, constant, helper,
# or assert defined above it (TS38PP_FILES/TS38PP_OVERLAYS in particular
# stay exactly as pinned by test_ts38pp_family_files_exist's `== 7`) -- same
# discipline as the TS38GRID section's own "Purely additive" note.
# =============================================================================

TS38DENSE_SIZES = [2154, 10000, 46416, 146780, 215443]

# n -> (eval_every, max_steps, min_steps=ceil(n/128)) -- identical to
# TS38GRID_PINNED's entries for these same 5 sizes above (verified by
# reading ts38_base_n<N>.yaml directly, not copied blind).
TS38DENSE_PINNED = {
    2154: (5, 1000, 17),
    10000: (10, 2000, 79),
    46416: (55, 11000, 363),
...
# (arm, n, eval_every, max_steps, min_steps) for every one of the 15 files.
TS38DENSE_OVERLAYS = [
    (arm, n, *TS38DENSE_PINNED[n]) for arm in ("base", "pp", "fs") for n in TS38DENSE_SIZES
]

_TS38DENSE_PATH_TMPL = {
    "base": "sweeps/ts38/ts38_base_n{n}.yaml",
    "pp": "sweeps/ts38/ts38pp_pretaught_n{n}.yaml",
    "fs": "sweeps/ts38/ts38fs_dense_i1000_n{n}.yaml",
}
_TS38DENSE_BASE_FILE = {
    "base": "ts38_base.yaml",
    "pp": "ts38pp_pretaught.yaml",
    "fs": "ts38fs_target.yaml",
}
_TS38DENSE_RUN_ID_TMPL = {
    "base": "evt-ts38-base-n{n}",
    "pp": "evt-ts38pp-pretaught-n{n}",
    "fs": "evt-ts38fs-i1000-n{n}-s316",
}
# The fs arm has no committed-overlay run-id pattern anywhere above (the
# 55-cell grid is generated, not committed -- see the note two sections up);
# this is this family's own.
_TS38DENSE_FS_RUN_ID_PATTERN = re.compile(r"^evt-ts38fs-i1000-n\d+-s316$")
# Every family whose run-id namespace a ts38dense id must NOT collide with,
# keyed by family name so a per-arm test can compute "own" vs "every other".
_TS38DENSE_ALL_FAMILY_PATTERNS = {
    "ts38_base": _TS38_RUN_ID_PATTERN,
    "ts38mw": _TS38MW_RUN_ID_PATTERN,
    "ts38pf": _TS38PF_RUN_ID_PATTERN,
    "ts38pp": _TS38PP_RUN_ID_PATTERN,
    "ts38fs_parent": _TS38FS_PARENT_RUN_ID_PATTERN,
    "ts38fs_dense": _TS38DENSE_FS_RUN_ID_PATTERN,
}
_TS38DENSE_OWN_FAMILY_KEY = {"base": "ts38_base", "pp": "ts38pp", "fs": "ts38fs_dense"}
# train_target.py's ARM_REGIME (see the ARM-LETTER NOTE in ts38_base.yaml,
# read at the top of this file's ts38 section): base -> teach, pp/fs -> elicit.
_TS38DENSE_REGIME = {"base": "teach", "pp": "elicit", "fs": "elicit"}

TS38DENSE_ALL_FILES = [
    _TS38DENSE_PATH_TMPL[arm].format(n=n) for arm in ("base", "pp", "fs") for n in TS38DENSE_SIZES
]


def test_ts38dense_family_files_exist() -> None:
    assert len(TS38DENSE_ALL_FILES) == 15
    for rel in TS38DENSE_ALL_FILES:
        assert (CONFIGS / rel).is_file(), rel


@pytest.mark.parametrize(("arm", "n", "eval_every", "max_steps", "min_steps"), TS38DENSE_OVERLAYS)
def test_ts38dense_overlay_file_exists(
    arm: str, n: int, eval_every: int, max_steps: int, min_steps: int
) -> None:
    path = _TS38DENSE_PATH_TMPL[arm].format(n=n)
    assert (CONFIGS / path).is_file(), path


@pytest.mark.parametrize(("arm", "n", "eval_every", "max_steps", "min_steps"), TS38DENSE_OVERLAYS)
def test_ts38dense_run_id_matches_own_pattern_and_no_other_family(
    arm: str, n: int, eval_every: int, max_steps: int, min_steps: int
) -> None:
    path = _TS38DENSE_PATH_TMPL[arm].format(n=n)
    raw = yaml.safe_load((CONFIGS / path).read_text())
    run_id = raw["run_id"]
    assert run_id == _TS38DENSE_RUN_ID_TMPL[arm].format(n=n)
    own_key = _TS38DENSE_OWN_FAMILY_KEY[arm]
    assert _TS38DENSE_ALL_FAMILY_PATTERNS[own_key].match(run_id), run_id
    for key, pattern in _TS38DENSE_ALL_FAMILY_PATTERNS.items():
        if key != own_key:
            assert not pattern.match(run_id), (arm, key, run_id)


def test_ts38dense_fs_run_id_pattern_rejects_known_lookalikes() -> None:
    """Explicit negative examples for _TS38DENSE_FS_RUN_ID_PATTERN -- the
    other run-id families whose prefixes are easy to typo into this one."""
    rejects = [
        "evt-ts38fs-tiny-i100-n21544-s316",
        "evt-ts38fs-tiny-i10-n21544-s316",
        "evt-ts38fs-tiny-i2-n21544-s316",
        "evt-ts38fs-parent-n1000",
        "evt-ts38fs-i1000-n21544-s1316",  # right install, wrong seed
        "evt-ts38fs-i4642-n21544-s316",  # right seed, wrong install
        "evt-ts38pf-preteachfmt-n2154",
        "evt-ts38pp-pretaught-n2154",
        "evt-ts38-base-n2154",
    ]
    for rid in rejects:
        assert not _TS38DENSE_FS_RUN_ID_PATTERN.match(rid), rid


@pytest.mark.parametrize(("arm", "n", "eval_every", "max_steps", "min_steps"), TS38DENSE_OVERLAYS)
def test_ts38dense_overlay_pinned_values(
    arm: str, n: int, eval_every: int, max_steps: int, min_steps: int
) -> None:
    path = _TS38DENSE_PATH_TMPL[arm].format(n=n)
    raw = yaml.safe_load((CONFIGS / path).read_text())
...
@pytest.mark.parametrize(("arm", "n", "eval_every", "max_steps", "min_steps"), TS38DENSE_OVERLAYS)
def test_ts38dense_min_steps_equals_ceil_n_over_128(
    arm: str, n: int, eval_every: int, max_steps: int, min_steps: int
) -> None:
    """min_steps must be the COMPUTED ceil(n/128), not just a literal that
    happens to match the table -- cross-checked against the same
    require_full_epoch1_launch_check the launcher itself calls, same as
    TS38GRID's own version of this test above."""
    train_target = load("train_target")
    steps_per_epoch = math.ceil(n / 128)
    assert min_steps == steps_per_epoch
    train_target.require_full_epoch1_launch_check(True, min_steps, steps_per_epoch)
...
@pytest.mark.parametrize(("arm", "n", "eval_every", "max_steps", "min_steps"), TS38DENSE_OVERLAYS)
def test_ts38dense_overlay_ceiling_rules(
    arm: str, n: int, eval_every: int, max_steps: int, min_steps: int
) -> None:
    """Two independent ceiling checks on the MERGED config: the >=20-epoch
    cost-ceiling floor, and the earliest-possible-stop arithmetic (max_steps
    must cover min_steps PLUS k more eval_every-spaced evaluations) -- same
    pair TS38GRID's own ceiling-rules test checks above."""
    path = _TS38DENSE_PATH_TMPL[arm].format(n=n)
    cfg = load_config(CONFIGS / _TS38DENSE_BASE_FILE[arm], CONFIGS / path)
    t = cfg["train"]
    assert t["max_steps"] >= 20 * t["stopping"]["min_steps"]
    assert t["max_steps"] >= t["stopping"]["min_steps"] + t["stopping"]["k"] * t["eval_every"]


@pytest.mark.parametrize("n", TS38DENSE_SIZES)
def test_ts38dense_arms_agree_on_merged_schedule_and_recipe(n: int) -> None:
    """The three arms at a given size share one recipe -- only theta0
    differs -- so their MERGED schedule AND shared-recipe fields
    (n_examples/eval_every/max_steps/min_steps/lr/batch_size/eps_nats/k/
    lora r&alpha) must be identical, not just their own overlay's raw
    literals."""
    merged = {}
    for arm in ("base", "pp", "fs"):
        path = _TS38DENSE_PATH_TMPL[arm].format(n=n)
        cfg = load_config(CONFIGS / _TS38DENSE_BASE_FILE[arm], CONFIGS / path)
        merged[arm] = (
            cfg["data"]["n_examples"],
    ...
    assert merged["base"] == merged["pp"] == merged["fs"], merged


@pytest.mark.parametrize("n", TS38DENSE_SIZES)
def test_ts38dense_pp_overlay_match_data_order_with_is_base_anchor(n: int) -> None:
    path = _TS38DENSE_PATH_TMPL["pp"].format(n=n)
    raw = yaml.safe_load((CONFIGS / path).read_text())
    assert raw["experiment"]["match_data_order_with"] == f"evt-ts38-base-n{n}"


@pytest.mark.parametrize("n", TS38DENSE_SIZES)
def test_ts38dense_base_overlay_carries_no_match_data_order_with_key(n: int) -> None:
    """The reused base overlay IS the G7 anchor -- its own raw yaml must not
    set match_data_order_with at all; the merged value stays ts38_base.yaml's
    own null placeholder. Re-verifies the ALREADY-EXISTING ts38grid file,
    since ts38dense's pp arm depends on this being true."""
    path = _TS38DENSE_PATH_TMPL["base"].format(n=n)
    raw = yaml.safe_load((CONFIGS / path).read_text())
    assert "match_data_order_with" not in raw.get("experiment", {})
    cfg = load_config(CONFIGS / "ts38_base.yaml", CONFIGS / path)
    assert cfg["experiment"]["match_data_order_with"] is None
...
@pytest.mark.parametrize("n", TS38DENSE_SIZES)
def test_ts38dense_fs_overlay_shape(n: int) -> None:
    """The fs overlay carries NO match_data_order_with (deliberately, same
    as every other ts38fs cell -- see ts38fs_target.yaml's "NO G7 PAIRING"
    header), points at the i=1000 LoRA parent, and pins its seed to 316 in
    BOTH data and train (mirrors generate_ts38fs_overlays.py's own
    overlay_doc shape: data.seed/train.seed set to the SAME value)."""
    path = _TS38DENSE_PATH_TMPL["fs"].format(n=n)
    raw = yaml.safe_load((CONFIGS / path).read_text())
    assert "match_data_order_with" not in raw.get("experiment", {})
    assert raw["experiment"]["parent_run_id"] == "evt-ts38fs-parent-n1000"
    assert raw["data"]["seed"] == 316
    assert raw["train"]["seed"] == 316
    cfg = load_config(CONFIGS / "ts38fs_target.yaml", CONFIGS / path)
    assert cfg["experiment"]["match_data_order_with"] is None
    assert cfg["experiment"]["parent_run_id"] == "evt-ts38fs-parent-n1000"
    assert cfg["data"]["seed"] == cfg["train"]["seed"] == 316


@pytest.mark.parametrize("n", TS38DENSE_SIZES)
def test_ts38dense_fs_filename_does_not_match_ts38fs_generator_glob(n: int) -> None:
    """launch_ts38fs_family.sh:501 hard-fails unless EXACTLY 55 files under
    configs/sweeps/ts38/ match `ts38fs_i*_n*_s*.yaml`. These 5 hand-committed
    dense overlays use a different prefix (`ts38fs_dense_`) specifically so
    they are invisible to that glob/count guard."""
    path = _TS38DENSE_PATH_TMPL["fs"].format(n=n)
    name = Path(path).name
    assert not fnmatch.fnmatch(name, "ts38fs_i*_n*_s*.yaml"), name


def test_ts38dense_fs_ts38fs_generator_glob_checked_in_count_unchanged() -> None:
    """The generator's own 55-file glob must still find exactly 0 files
    CHECKED INTO GIT after this family lands -- those 55 files are written
    at launch time by generate_ts38fs_overlays.py and deliberately never
    committed. Checked via `git ls-files`, not an on-disk glob, so a box
    that has already run launch_ts38fs_family.sh (its own stage-3 comment:
    "expect `git status` on a box to show 55 untracked" files) never trips
    this on a dirty working tree."""
    result = subprocess.run(
        ["git", "ls-files", "--", "sweeps/ts38/ts38fs_i*_n*_s*.yaml"],
        cwd=CONFIGS,
        capture_output=True,
        text=True,
        check=True,
...
# The 5 shipped ts38fs target sizes (generate_ts38fs_overlays.SIZES) --
# hardcoded rather than imported, same "deliberately NOT here" discipline as
# the note after test_ts38fs_target_builds_a_manifest_with_correct_regime
# above: this file never imports the generator module.
_TS38FS_SHIPPED_SIZES = (1000, 4642, 21544, 100000, 316228)


def test_ts38dense_fs_sizes_disjoint_from_generated_ts38fs_grid() -> None:
    """evt-ts38fs-i1000-n<N>-s316 for N in TS38DENSE_SIZES must not collide
    with the (uncommitted, launch-time-generated) i=1000/s=316 row of the
    full 55-cell ts38fs grid, which uses the 5 SHIPPED sizes instead."""
    assert set(TS38DENSE_SIZES).isdisjoint(_TS38FS_SHIPPED_SIZES)


@pytest.mark.parametrize(("arm", "n", "eval_every", "max_steps", "min_steps"), TS38DENSE_OVERLAYS)
def test_ts38dense_overlay_builds_a_manifest_with_correct_regime(
    arm: str, n: int, eval_every: int, max_steps: int, min_steps: int
) -> None:
    train_target = load("train_target")
    path = _TS38DENSE_PATH_TMPL[arm].format(n=n)
    ...
    assert manifest["regime"] == _TS38DENSE_REGIME[arm]


def _every_committed_ts38_run_id() -> set[str]:
    """Every run_id already declared by another TS38* section in this file --
    read from the committed yaml (not reconstructed from a template string),
    so a stale format string couldn't mask a real collision."""
    paths: list[str] = [
        *TS38_RUN_ID_FILES,
        *TS38MW_FILES,
        *TS38PF_FILES,
    ...
    for arm in ("ts38", "ts38mw", "ts38pf"):
        for n in TS38GRID_SIZES:
            paths.append(_TS38GRID_PATH_TMPL[arm].format(n=n))
    return {yaml.safe_load((CONFIGS / rel).read_text())["run_id"] for rel in paths}


def test_ts38dense_run_ids_disjoint_from_every_other_ts38_family() -> None:
    """Only pp/fs mint NEW run ids here -- the base arm is a deliberate
    REUSE of the already-shipped evt-ts38-base-n<N> run (ts38grid built it
    for these exact 5 sizes; see this section's header), so its id is
    expected to already be in `shipped`, not disjoint from it. That reuse
    identity is asserted separately by
    test_ts38dense_run_id_matches_own_pattern_and_no_other_family (which
    checks the base overlay's run_id equals _TS38DENSE_RUN_ID_TMPL["base"]
    exactly) -- this test only covers the two genuinely NEW namespaces."""
...
# (EXPERIMENTS.md, decisions.md "ts38tr" entry): two target arms, k6 and k7,
# both VERBATIM copies of ts38mt_pp.yaml's recipe (same task/data/lora/train
# blocks byte-identical, arms differ only in theta0 like every ts38(*)
# family), whose theta0 is NOT another training run's checkpoint but a
# scripts/truncate_lora_parent.py BUILD: evt-ts38mt-base-n316228's converged
# LoRA adapter merged into blocks 0..K-1 only, blocks K.. left as untouched
# base -- k7 keeps 7 of 8 blocks (merged through the last block the adapter
# was trained on), k6 keeps 6 (one block short). Unlike every earlier
# ts38(*) family, the parents (evt-ts38tr-k6-parent / evt-ts38tr-k7-parent)
# have NO committed yaml -- they are minted by the build script directly, so
# every test below that needs their run_id uses the python string constant,
# never yaml.safe_load(...)["run_id"]. Only 3 of ts38mt's 10 sizes are
# covered (the smallest bracket: 1000/2154/4642), not the full grid -- this
# family is a positive-control probe, not a densified sweep.
# =============================================================================

TS38TR_SIZES = [1000, 2154, 4642]

# n -> (eval_every, max_steps, min_steps=ceil(n/128)); identical numbers to
# TS38MT_PINNED's entries for these same 3 sizes (cross-checked against the
# committed ts38mt_pp_n<size>.yaml overlays directly, not copied blind).
TS38TR_PINNED = {
    1000: (5, 1000, 8),
    2154: (5, 1000, 17),
    4642: (5, 1000, 37),
...
_TS38TR_PATH_TMPL = {
    ...
    "k7": "sweeps/ts38/ts38tr_k7_n{n}.yaml",
}
_TS38TR_BASE_FILE = {"k6": "ts38tr_k6.yaml", "k7": "ts38tr_k7.yaml"}
_TS38TR_RUN_ID_TMPL = {"k6": "evt-ts38tr-k6-n{n}", "k7": "evt-ts38tr-k7-n{n}"}
# train_target.py's ARM_REGIME (see the ARM-LETTER NOTE in ts38_base.yaml):
# k7 mirrors ts38mt_pp.yaml's own arm (A, elicit); k6 is arm B (teach).
_TS38TR_REGIME = {"k6": "teach", "k7": "elicit"}
_TS38TR_ARM_LETTER = {"k6": "B", "k7": "A"}
# Minted by scripts/truncate_lora_parent.py, no committed yaml -- python
# string constants, deliberately never read via yaml.safe_load(...)["run_id"].
_TS38TR_PARENT_RUN_ID_FOR_ARM = {
    "k6": "evt-ts38tr-k6-parent",
    "k7": "evt-ts38tr-k7-parent",
}
...
# parents are never committed yaml, so their ids must NOT match this family's
# own run-id pattern (verified by test_ts38tr_run_id_pattern_rejects_known_
# lookalikes below), same discipline as ts38mt rejecting its own fmt-parent id.
_TS38TR_RUN_ID_PATTERN = re.compile(r"^evt-ts38tr-(k6|k7)(-n\d+)?$")
# Every family whose run-id namespace a ts38tr id must NOT collide with --
# includes ts38mt's pattern too, since ts38tr's parent is BUILT FROM one of
# ts38mt's own runs (evt-ts38mt-base-n316228) and the two families' configs
# live side by side in the same sweeps/ts38/ directory.
_TS38TR_OTHER_FAMILY_PATTERNS = (
    _TS38_FAMILY_REGEX,
    _TS38MW_RUN_ID_PATTERN,
    _TS38PF_RUN_ID_PATTERN,
    _TS38PP_RUN_ID_PATTERN,
    _TS38FS_PARENT_RUN_ID_PATTERN,
    _TS38DENSE_FS_RUN_ID_PATTERN,
    _TS38MT_RUN_ID_PATTERN,
)

# Exact (not subset) diffs against each arm's true closest sibling --
# ts38mt_pp.yaml, VERBATIM copied per this family's own header, not
# ts38_base.yaml (the ts38mt precedent's sibling). k7 is arm A same as
# ts38mt_pp.yaml itself, so it carries NO arm diff; k6 is arm B, so it does.
_TS38TR_K7_VS_TS38MT_PP_EXACT = {"run_id", "experiment.parent_run_id"}
_TS38TR_K6_VS_TS38MT_PP_EXACT = {"run_id", "experiment.arm", "experiment.parent_run_id"}
_TS38TR_K6_VS_K7_EXACT = {"run_id", "experiment.arm", "experiment.parent_run_id"}
...
# ---- the two target bases (k6 / k7), diffed against ts38mt_pp.yaml --------


def test_ts38tr_k7_vs_ts38mt_pp_exact_diff() -> None:
    pp = yaml.safe_load((CONFIGS / "ts38mt_pp.yaml").read_text())
    k7 = yaml.safe_load((CONFIGS / "ts38tr_k7.yaml").read_text())
    assert _leaf_diffs(pp, k7) == _TS38TR_K7_VS_TS38MT_PP_EXACT


def test_ts38tr_k6_vs_ts38mt_pp_exact_diff() -> None:
    pp = yaml.safe_load((CONFIGS / "ts38mt_pp.yaml").read_text())
    k6 = yaml.safe_load((CONFIGS / "ts38tr_k6.yaml").read_text())
    assert _leaf_diffs(pp, k6) == _TS38TR_K6_VS_TS38MT_PP_EXACT


def test_ts38tr_k6_vs_k7_exact_diff() -> None:
    k6 = yaml.safe_load((CONFIGS / "ts38tr_k6.yaml").read_text())
    k7 = yaml.safe_load((CONFIGS / "ts38tr_k7.yaml").read_text())
    assert _leaf_diffs(k6, k7) == _TS38TR_K6_VS_K7_EXACT


@pytest.mark.parametrize("path", TS38TR_TARGET_BASE_FILES)
def test_ts38tr_snapshots_pinned(path: str) -> None:
    cfg = load_config(CONFIGS / path, None)
    assert cfg["train"]["snapshots"] == {"n": 32, "dense_until": 8}


@pytest.mark.parametrize(("arm", "path"), TS38TR_TARGET_BASE_PARAMS)
def test_ts38tr_target_base_theta0_and_arm(arm: str, path: str) -> None:
    cfg = load_config(CONFIGS / path, None)
    assert cfg["run_id"] == f"evt-ts38tr-{arm}"
    assert cfg["experiment"]["parent_run_id"] == _TS38TR_PARENT_RUN_ID_FOR_ARM[arm]
    assert cfg["experiment"]["arm"] == _TS38TR_ARM_LETTER[arm]
    assert cfg["experiment"]["parent_required_gates"] == []
    assert cfg["experiment"]["require_full_epoch1"] is True
    assert cfg["experiment"]["match_data_order_with"] is None
    assert _TS38TR_RUN_ID_PATTERN.match(cfg["run_id"])


@pytest.mark.parametrize(("arm", "path"), TS38TR_TARGET_BASE_PARAMS)
def test_ts38tr_target_base_builds_a_manifest_with_correct_regime(arm: str, path: str) -> None:
    train_target = load("train_target")
    cfg = load_config(CONFIGS / path, None)
    manifest = train_target.manifest_fields(
        cfg,
    ...
    assert manifest["regime"] == _TS38TR_REGIME[arm]


# ---- the 6 overlays (2 arms x 3 sizes) -------------------------------------
...
@pytest.mark.parametrize(("arm", "n", "eval_every", "max_steps", "min_steps"), TS38TR_OVERLAYS)
def test_ts38tr_overlay_pinned_values(
    arm: str, n: int, eval_every: int, max_steps: int, min_steps: int
) -> None:
    path = _TS38TR_PATH_TMPL[arm].format(n=n)
    raw = yaml.safe_load((CONFIGS / path).read_text())
    assert raw["data"]["n_examples"] == n
    assert raw["train"]["eval_every"] == eval_every
    assert raw["train"]["max_steps"] == max_steps
...
@pytest.mark.parametrize(("arm", "n", "eval_every", "max_steps", "min_steps"), TS38TR_OVERLAYS)
def test_ts38tr_overlay_sets_exactly_the_expected_keys(
    arm: str, n: int, eval_every: int, max_steps: int, min_steps: int
) -> None:
    """Same shape as every existing ts38(*) overlay: run_id +
    experiment.match_data_order_with + data.n_examples +
    train.{eval_every,max_steps,stopping.min_steps} -- nothing else, on the
    RAW (un-merged) file."""
    path = _TS38TR_PATH_TMPL[arm].format(n=n)
    raw = yaml.safe_load((CONFIGS / path).read_text())
    assert set(raw) == {"run_id", "experiment", "data", "train"}
...
@pytest.mark.parametrize(("arm", "n", "eval_every", "max_steps", "min_steps"), TS38TR_OVERLAYS)
def test_ts38tr_overlay_run_id_matches_filename_and_no_other_family(
    arm: str, n: int, eval_every: int, max_steps: int, min_steps: int
) -> None:
    path = _TS38TR_PATH_TMPL[arm].format(n=n)
    raw = yaml.safe_load((CONFIGS / path).read_text())
    assert raw["run_id"] == _TS38TR_RUN_ID_TMPL[arm].format(n=n)
    assert _TS38TR_RUN_ID_PATTERN.match(raw["run_id"])
    for pattern in _TS38TR_OTHER_FAMILY_PATTERNS:
        assert not pattern.match(raw["run_id"]), (arm, raw["run_id"])


def test_ts38tr_run_id_pattern_rejects_known_lookalikes() -> None:
    """Explicit negative examples for _TS38TR_RUN_ID_PATTERN -- other
    families' ids and, critically, this family's OWN parent ids (which have
    no committed yaml and must never be mistaken for a target base/overlay)."""
    rejects = [
        "evt-ts38-base-n1000",
        "evt-ts38pp-pretaught-n1000",
        "evt-ts38pf-preteachfmt-n1000",
        "evt-ts38mw-pretaught-n1000",
        "evt-ts38mt-pp-n1000",
        "evt-ts38mt-base-n1000",
        "evt-ts38mt-fmt-parent",
        "evt-ts38tr-k6-parent",
        "evt-ts38tr-k7-parent",
        "evt-ts38tr-k8",
        "evt-ts38tr-k6n1000",
        "evt-ts38tr-k6-n",
    ]
    for rid in rejects:
        assert not _TS38TR_RUN_ID_PATTERN.match(rid), rid


def test_ts38tr_parent_ids_are_not_own_pattern_or_any_other_family() -> None:
    """The build-script-minted parent ids exist only as python string
    constants (no yaml) -- pin here that they don't collide with this
    family's own overlay pattern or any earlier family's."""
    for parent_id in _TS38TR_PARENT_RUN_ID_FOR_ARM.values():
        assert not _TS38TR_RUN_ID_PATTERN.match(parent_id), parent_id
        for pattern in _TS38TR_OTHER_FAMILY_PATTERNS:
            assert not pattern.match(parent_id), (parent_id, pattern)


@pytest.mark.parametrize(("arm", "n", "eval_every", "max_steps", "min_steps"), TS38TR_OVERLAYS)
def test_ts38tr_overlay_match_data_order_with_is_shipped_base_anchor(
    arm: str, n: int, eval_every: int, max_steps: int, min_steps: int
) -> None:
    path = _TS38TR_PATH_TMPL[arm].format(n=n)
    raw = yaml.safe_load((CONFIGS / path).read_text())
...
@pytest.mark.parametrize(("arm", "n", "eval_every", "max_steps", "min_steps"), TS38TR_OVERLAYS)
def test_ts38tr_min_steps_equals_ceil_n_over_128(
    arm: str, n: int, eval_every: int, max_steps: int, min_steps: int
) -> None:
    train_target = load("train_target")
    steps_per_epoch = math.ceil(n / 128)
...
@pytest.mark.parametrize(("arm", "n", "eval_every", "max_steps", "min_steps"), TS38TR_OVERLAYS)
def test_ts38tr_overlay_ceiling_covers_earliest_possible_stop(
    arm: str, n: int, eval_every: int, max_steps: int, min_steps: int
) -> None:
    path = _TS38TR_PATH_TMPL[arm].format(n=n)
    cfg = load_config(CONFIGS / _TS38TR_BASE_FILE[arm], CONFIGS / path)
    t = cfg["train"]
    assert t["max_steps"] >= t["stopping"]["min_steps"] + t["stopping"]["k"] * t["eval_every"]


@pytest.mark.parametrize("n", TS38TR_SIZES)
def test_ts38tr_arms_agree_on_merged_schedule_and_recipe(n: int) -> None:
    """k6/k7 share one recipe at a given size -- only theta0 differs -- so
    their MERGED shared-recipe fields must be identical, not just their own
    overlay's raw literals."""
    merged = {}
    for arm in ("k6", "k7"):
        path = _TS38TR_PATH_TMPL[arm].format(n=n)
        cfg = load_config(CONFIGS / _TS38TR_BASE_FILE[arm], CONFIGS / path)
    ...
    assert merged["k6"] == merged["k7"], merged


@pytest.mark.parametrize(("arm", "n", "eval_every", "max_steps", "min_steps"), TS38TR_OVERLAYS)
def test_ts38tr_overlay_builds_a_manifest_with_correct_regime(
    arm: str, n: int, eval_every: int, max_steps: int, min_steps: int
) -> None:
    train_target = load("train_target")
    path = _TS38TR_PATH_TMPL[arm].format(n=n)
...
def _ts38mt_committed_run_ids() -> set[str]:
    """Every run_id ts38mt already declared -- NOT covered by
    ``_every_committed_ts38_run_id()`` (that helper predates ts38mt), but the
    one realistic collision surface for ts38tr (its parent is built FROM an
    ts38mt run and its configs live in the same directory)."""
    return {yaml.safe_load((CONFIGS / rel).read_text())["run_id"] for rel in TS38MT_ALL_FILES}


def test_ts38tr_run_ids_disjoint_from_every_other_ts38_family() -> None:
    """None of ts38tr's run ids (2 target bases, 6 overlays, 2 build-script
    parent ids) may already be claimed by any earlier ts38(*) family --
    including ts38mt, whose own helper does not cover it."""
    shipped = _every_committed_ts38_run_id() | _ts38mt_committed_run_ids()
    tr_files = [*TS38TR_TARGET_BASE_FILES, *TS38TR_ALL_OVERLAY_FILES]
    tr_ids = {yaml.safe_load((CONFIGS / rel).read_text())["run_id"] for rel in tr_files}
    tr_ids |= set(_TS38TR_PARENT_RUN_ID_FOR_ARM.values())
    assert tr_ids.isdisjoint(shipped), tr_ids & shipped
